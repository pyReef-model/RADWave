...
def test_waveAnalysis_class_building():
    ...
    wclass = RADWave.waveAnalysis(
        altimeterURL="tests/testURLs.txt",
...
def test_altimeter_processing():
    ...
    # wclass.processAltimeterData(altimeter_pick="all", saveCSV='tests/altimeterData1.csv')
    assert (
        pytest.approx(wclass.wh.mean(), rel=1e-3) == 2.386
    ), "test failed because extracted mean wave height missmatch"
    assert (
        pytest.approx(wclass.ws.mean(), rel=1e-3) == 8.221
...
def test_seasonal_characteristics():
    ...
    wclass.readAltimeterData(saveCSV="tests/altimeterData1.csv")
    ts = wclass.generateTimeSeries(days=30)
    wh_season = wclass.computeSeasonalCharacteristics(
        series="wh", time=[1998, 2008], lonlat=None, fsave=None, plot=False
    )
